...
export const HomeScreen: React.FC<HomeScreenProps> = ({ selectedMode, onModeChange, onDestinationSelect }) => (
  <section className="screen-panel home-screen" aria-labelledby="home-heading">
    <div className="eyebrow"><span className="status-dot" /> City network online <span className="eyebrow-divider" /> 06:42 local</div>
    <div className="home-heading-row">
      <div>
        <h2 id="home-heading">Where are you<br /><span>going today?</span></h2>
        <p className="lede">One calm journey across every layer of Aurelia.</p>
      </div>
      <div className="network-mark" aria-hidden="true"><Navigation /></div>
    </div>

    <button className="search-field" type="button" aria-label="Search for a destination">
      <span className="search-copy"><MapPin aria-hidden="true" /><span>Search a destination</span></span>
      <span className="search-shortcut">⌘ K</span>
    </button>
    <button className="voice-button" type="button" aria-label="Start voice search">
      <Mic aria-hidden="true" />
      <span><strong>Speak your destination</strong><small>Hands-free search</small></span>
      <ArrowRight aria-hidden="true" />
    </button>

    <div className="section-heading"><div><span className="section-kicker">01 / Choose your mode</span><h3>How do you want to travel?</h3></div><span className="selection-count">{modes.filter((mode) => mode.id === selectedMode).length ? '1 selected' : ''}</span></div>
    <div className="mode-grid" role="group" aria-label="Transport modes">
      {modes.map(({ id, label, detail, icon: Icon }) => {
        const selected = selectedMode === id;
        return <button key={id} className={`mode-chip ${selected ? 'is-selected' : ''}`} onClick={() => onModeChange(id)} aria-pressed={selected}>
          <span className="mode-icon"><Icon aria-hidden="true" /></span><span><strong>{label}</strong><small>{detail}</small></span>{selected && <Check className="mode-check" aria-hidden="true" />}
        </button>;
      })}
    </div>

    <div className="section-heading destinations-heading"><div><span className="section-kicker">02 / Quick destinations</span><h3>Go somewhere familiar</h3></div><button className="text-button" type="button">Saved places <ChevronRight aria-hidden="true" /></button></div>
    <div className="destination-grid">
      {destinations.map(({ id, label, note, eta, icon: Icon }) => <button key={id} className="destination-card" onClick={() => onDestinationSelect(id)}>
        <span className="destination-icon"><Icon aria-hidden="true" /></span><span className="destination-text"><strong>{label}</strong><small>{note}</small></span><span className="destination-eta"><Clock3 aria-hidden="true" />{eta}</span><ChevronRight className="destination-arrow" aria-hidden="true" />
      </button>)}
    </div>
    <p className="helper-line"><Headphones aria-hidden="true" /> New here? Start with a quick destination. We will plan the rest.</p>
  </section>
);
